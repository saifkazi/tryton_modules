...
from trytond.modules.purchase.purchase import Purchase
...
class QualityControl(ModelSQL, ModelView):
    "QualityControl"
    
    __name__ = "production.qualitycontrol"
    

    code = fields.Char('TR no.')
    purchase = fields.Many2One('purchase.purchase', 'Purchase', domain=[('is_client', '=', True)] ,select=True, required=True)
    ...
    image = fields.Binary('Image', depends=['test_name'] , states = {'invisible': ~Eval('test_name').in_(['dft', 'adhesion']) })
    


    batch_qty = fields.Integer('Batch qty')
    sample_qty = fields.Integer('Sample qty')    

    report_by = fields.Many2One('company.employee', 'Generated By')
    test_result = fields.Selection('on_change_with_test_result', "Test Result")

    @fields.depends('test_name')
    def on_change_with_test_result(self , name=None):
        if self.test_name == 'dft':
            
            return [('nos', 'NOS')]

        if self.test_name == 'adhesion':
            
            return [('pass', 'Pass'),('fail', 'Fail')]  

        if self.test_name == 'color':
            
            return [('black', 'Black'),('gray', 'Gray')]

        if self.test_name == 'visual':
            
            return [('ok', 'Ok'),('not_ok', 'Not Ok')]
