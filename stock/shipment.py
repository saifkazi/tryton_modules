...
class ShipmentIn(ShipmentMixin, Workflow, ModelSQL, ModelView):
    ...
    is_client = fields.Boolean('Client',
        help="Shipment From Client")
    

    chaalan_date = fields.Date('Chaalan Date' , states = {'invisible': ~Eval('is_client', True) } )
    ...
    chaalan_no = fields.Char("Chaalan No.", states = {'invisible': ~Eval('is_client', True)  } ,
        help="The supplier's identifier for the shipment.")
    
    order_ref = fields.Function(fields.Char("Order Reference", states = {'invisible': ~Eval('is_client', True) , 'required': ~Eval('is_client', True) } ), 'get_order_ref') 
    
    prefix_party = fields.Char("Party Prefix", states = {'invisible': ~Eval('is_client', True) } )
    ...
    def on_change_with_is_client(self , name=None):
        
        if self.supplier == None:
            return False
        else:
            return self.supplier.is_client
    
    @fields.depends('supplier')
    def on_change_with_prefix_party(self , name=None):
        
        if self.supplier == None:
            return ""
        else:
            return self.supplier.party_prefix
            
   
    def get_order_ref(self, name=None):

        return  self.prefix_party+"/"+str(self.chaalan_no)
...
class ShipmentOut(ShipmentAssignMixin, Workflow, ModelSQL, ModelView):
    ...
    eway_bill_url = fields.Char("Eway Bill", readonly=True)
    eway_bill_no = fields.Char("Eway Bill No", readonly=True)
    ...
    @classmethod
    @ModelView.button
    def eway(cls, shipments):
        pool =  Pool()
        company_id = Transaction().context.get('company')
        Company = pool.get('company.company')
        gst = Company(company_id).party.gst_no
        if len(gst) == 0:
            raise UserError("Please Set the Valid GST no. in current Company")
        else:
            url = 'https://clientbasic.mastersindia.co/ewayBillsGenerate'
            request = {
                ...
                "document_number": "04455323",
                "document_date": "09/09/2020",
                "gstin_of_consignor": "05AAABB0639G1Z8",
                "legal_name_of_consignor": "Masters India",
                "address1_of_consignor": " UNIT-1, SECTOR-3, NOIDA",
                "place_of_consignor": "NOIDAs",
                ...
                "legal_name_of_consignee": "A R ENTERPRISES, VARANASI (U.P.)",
                "address1_of_consignee": "13/32, TARNA KHAS, SHIVPUR, VARANASI -221001 (UTTAR PRADESH)",
                "place_of_consignee": "VARANASI",
                "pincode_of_consignee": 221001,
                "state_of_supply": "Uttar Pradesh",
                "actual_to_state_name": "Uttar Pradesh",
                "transaction_type": 1,
                "other_value": 1591.76,
                "taxable_amount": 106117,
                "total_invoice_value": 199690.96,
                "cgst_amount": 14633.53,
                "sgst_amount": 14633.53,
                "igst_amount": 0,
                "cess_amount": 62715.14,
                "cess_nonadvol_value": 0,
                "transporter_id": "05AAABB0639G1Z8",
                "transportation_mode": "Road",
                "transportation_distance": 0,
                "vehicle_number": "up16bw2236",
            ...
            if data['results']['status'] == 'Success':
                print(data['results']['message']['url'])
                cls.write(shipments, {
                'eway_bill_url': 'http://'+data['results']['message']['url'],
                })

                cls.write(shipments, {
                'eway_bill_no': data['results']['message']['ewayBillNo'],
                })
            else:
                pass
